...
const ArchiveProjectModal = ({ 
  ...
  setProjects, 
  projects,
  setSelectedProjectForSidebar // Sidebar close karne ke liye optional
}) => {
  ...
  const handleArchive = () => {
    ...
    setActiveModal(null);
    // Agar sidebar khula hai toh usay band kar dein kyunke project archive ho gaya
      ...
      <div className="bg-white rounded-[2.5rem] w-full max-w-sm p-10 text-center shadow-2xl animate-in zoom-in slide-in-from-bottom-4 duration-300">
        ...
        <div className="flex flex-col gap-3">
          ...
          <button
            ...
            className="w-full py-4 border border-slate-100 rounded-2xl font-black text-[10px] text-slate-400 uppercase tracking-widest hover:bg-slate-50 transition-all active:scale-95"
          >
            Go Back
          </button>
        </div>
