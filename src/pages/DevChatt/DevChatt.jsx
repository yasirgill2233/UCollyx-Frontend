...
const getChatGroupLabel = (dateString) => {
  ...
  console.log("Grouping Date Check for:", dateString);
...
const DevChat = () => {
  ...
  const messages = useMemo(() => {
    ...
    console.log("Raw Messages for Memoization:#########################################################", rawMessages);
  ...
  const groupedMessages = useMemo(() => {
    ...
    messages.forEach((msg) => {
      ...
      const dateLabel = getChatGroupLabel(msg.sent_at || msg.created_at || msg.timestamp);
      if (!groups[dateLabel]) {
        groups[dateLabel] = [];
      }
  ...
  return (
    ...
    Mobile par absolute overlay drawer banega, desktop par inline layouts follow karega */}
      <div
        className={`
...
`}
      >
        ...
        <div className="h-[68px] px-6 flex justify-between items-center border-b border-slate-100 flex-shrink-0">
          ...
          <div className="flex items-center gap-1">
            {/* Mobile-only Close Button: Jab drawer open ho to mobile par band karne ke liye */}
            <button
        ...
        {/* Sidebar Body Content Streams */}
        <div className="flex-1 overflow-y-auto p-4 space-y-8 custom-scrollbar">
          {/* Jump To / Search Trigger Node */}
          ...
          <div className="hidden sm:flex -space-x-2 mr-1 md:mr-2">
            {activeChat?.type === "channel" && (
              ...
              >
                {channelMembers
                  ...
                  .map((i) => (
                    ...
                    >
                      {i.User.avatar_url ? (
                        <img
                          src={import.meta.env.VITE_API_URL + i.User.avatar_url}
                ...
                {channelMembers?.filter((m) => m.User.status === "active").length > 4 && (
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-8 bg-[#f5f5f593]">
        {isChatLoading ? (
          ...
          <>
            ...
            {Object.entries(groupedMessages).map(([dateLabel, groupMessages]) => (
                    ...
                    >
                      ...
                      <div
                        ...
                        className={`w-8 h-8 md:w-10 md:h-10 rounded-xl md:rounded-2xl flex-shrink-0 flex items-center justify-center font-black text-xs shadow-sm cursor-pointer hover:scale-105 transition-transform overflow-hidden ${
                          msg.isMe ? "bg-blue-600 text-white" : "bg-slate-800 text-white"
                        }`}
                      >
                        {msg.avatar_url ? (
                          <img
                      ...
                      {/* Bubble Layout Block */}
                      <div className={`flex flex-col max-w-[85%] md:max-w-xl ${msg.isMe ? "items-end" : "items-start"}`}>
                          ...
                          {msg.type === "call" ? (
                            ...
                            >
                              <div className="flex items-center gap-3 md:gap-4">
                                <div
                                  className={`w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl flex-shrink-0 flex items-center justify-center ${
                                    msg.call_status === "active" ? "bg-white/20 animate-pulse" : "bg-slate-100"
                                  }`}
                                >
                                  {msg.call_status === "scheduled" ? (
                                    <Calendar size={18} className={msg.call_status === "active" ? "text-white" : "text-blue-600"} />
                                  ) : (
                                    <Video size={18} className={msg.call_status === "active" ? "text-white" : "text-indigo-600"} />
                                  )}
                                </div>

                                <div className="flex-1 min-w-0">
                                  <h4 className="text-xs md:text-[14px] font-black truncate">
                                    {msg.call_status === "active" ? "Live Meeting" : msg.call_status === "scheduled" ? "Scheduled Meeting" : "Call Ended"}
                                  </h4>
                                  <p className={`text-[9px] md:text-[10px] truncate ${msg.call_status === "active" ? "text-indigo-100" : "text-slate-500"}`}>
                                    {msg.call_status === "scheduled"
                              ...
                              <div className="mt-3 md:mt-4">
                                {(msg.call_status === "active" || (msg.call_status === "scheduled" && isMeetingLive(msg.scheduled_at))) ? (
                                  <button
                                    onClick={() => msg.call_status === "scheduled" ? handleJoinScheduledMeeting(msg) : setIsVideoModalOpen(true)}
                                    className={`w-full py-2 rounded-lg md:rounded-xl text-xs font-black transition-all flex items-center justify-center gap-2 ${
                                      msg.call_status === "active" ? "bg-white text-indigo-600 hover:bg-indigo-50" : "bg-green-600 text-white hover:bg-green-700"
                                    }`}
                                  >
                                  ...
                                  <div className="text-center py-2 bg-slate-50 rounded-xl border border-dashed border-slate-200">
                                    <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Waiting for Time</span>
                                  </div>
                                ) : null}
                              </div>
                              ...
                              {msg.call_status === "ended" && msg.transcript && (
                                <div className="mt-3 p-2.5 bg-slate-50 rounded-xl border border-slate-100">
                                  <p className="text-[10px] font-bold text-slate-400 mb-1 uppercase">Meeting Transcript</p>
                                  <p className="text-xs text-slate-600 line-clamp-2 italic">"{msg.transcript}"</p>
                                  {msg.audio_url && (
                                    <audio controls className="w-full h-8 mt-2 scale-90 origin-left">
                                      <source src={msg.audio_url} type="audio/mpeg" />
                                    </audio>
                                  )}
                                  <button className="mt-1.5 text-blue-600 text-[10px] font-black hover:underline">READ FULL SUMMARY</button>
                                </div>
                              )}
                            </div>
                          ) : (
                              ...
                              {msg.attachments && msg.attachments.length > 0 && (
                                <div className="flex flex-wrap gap-2 mt-2">
                                  {msg.attachments.map((file, idx) => {
                                    ...
                                    return (
                                      <div key={idx} className="w-full sm:max-w-[250px]">
                                        {isImage ? (
                                          ...
                                          >
                                            ...
                                            <div className="flex flex-col overflow-hidden min-w-0">
                                              <span className="text-[12px] font-bold text-slate-700 truncate w-32">{file.filename}</span>
      ...
      <div className="px-2 pb-3 pt-1 md:px-8 md:pb-8 bg-[#f5f5f593] flex-shrink-0 transition-all">
        <div className="relative bg-white border border-slate-200/80 rounded-2xl md:rounded-[24px] shadow-md md:shadow-xl md:shadow-slate-200/40 focus-within:border-blue-500/40 focus-within:ring-4 ring-blue-500/5 transition-all">
          {showMentions && activeChat?.type === "channel" && filteredUsers?.length > 0 && (
            <div className="absolute bottom-full left-0 mb-2 w-full sm:w-64 bg-white border border-slate-200 shadow-2xl rounded-xl overflow-hidden z-50">
              <div className="px-3 py-1.5 bg-slate-50 border-b border-slate-100">
                <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">People</span>
              </div>
              <div className="max-h-48 overflow-y-auto">
                {filteredUsers?.map((user) => (
                  <div
                    key={user?.User?.id}
                    onClick={() => selectMention(user?.User?.full_name)}
                  ...
                  >
                    <div className="rounded-full border border-blue-100 bg-blue-600 w-7 h-7 flex items-center justify-center text-white font-black text-[10px] uppercase overflow-hidden flex-shrink-0">
                      {user?.User.avatar_url ? (
                        <img src={import.meta.env.VITE_API_URL + user?.User.avatar_url} alt="Avatar" crossOrigin="anonymous" className="w-full h-full object-cover" />
                      ) : user?.User.full_name ? (
                    ...
                    <span className="text-xs font-semibold text-slate-700 truncate">{user?.User?.full_name}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {selectedFiles?.length > 0 && (
            <div className="flex flex-wrap gap-1.5 px-4 py-2 border-b border-slate-100 bg-slate-50/50 rounded-t-2xl">
              {selectedFiles.map((file, index) => (
                <div key={index} className="relative group bg-white border border-slate-200 rounded-lg p-1.5 flex items-center gap-2 pr-6 shadow-2xs">
                  <Paperclip size={12} className="text-blue-500" />
                  <span className="text-[11px] font-medium text-slate-600 truncate max-w-[100px]">{file.name}</span>
                  <button onClick={() => removeFile(index)} className="absolute top-1/2 -translate-y-1/2 right-1.5 text-slate-400 hover:text-red-500 transition-colors">
                    <X size={12} />
                  </button>
                </div>
              ))}
          ...
          <input type="file" ref={fileInputRef} onChange={handleFileChange} multiple className="hidden" accept="image/*,.pdf,.doc,.docx,.zip" />
          ...
          <div className="flex items-center justify-between px-3 py-2 md:px-6 md:py-4 bg-slate-50/60 rounded-b-2xl md:rounded-b-[23px] border-t border-slate-100/50">
            <div className="flex items-center gap-3.5 md:gap-5 text-slate-400">
              <button type="button" onClick={() => fileInputRef.current.click()} className="hover:text-blue-600 text-slate-500 transition-colors p-1 rounded-lg active:bg-slate-200/50">
                <Paperclip size={18} />
              </button>

              <div className="relative flex items-center" ref={emojiPickerRef}>
                <button type="button" onClick={() => setShowEmojiPicker(!showEmojiPicker)} className={`hover:text-blue-600 transition-colors p-1 rounded-lg ${showEmojiPicker ? "text-blue-600 bg-blue-50" : "text-slate-500"}`}>
                  <Smile size={18} />
                </button>
                {showEmojiPicker && (
                  <div className="absolute bottom-12 left-0 shadow-2xl border border-slate-100 rounded-xl overflow-hidden z-50 max-w-[280px] sm:max-w-none">
                    <EmojiPicker onEmojiClick={onEmojiClick} autoFocusSearch={false} theme="light" width={280} height={350} previewConfig={{ showPreview: false }} skinTonesDisabled />
                  </div>
                )}
              </div>

              <button type="button" className="hidden sm:inline-block hover:text-blue-600 text-slate-500 transition-colors p-0.5"><Mic size={18} /></button>
              <button type="button" onClick={handleStartMeeting} className="hidden sm:inline-block hover:text-blue-600 text-slate-500 transition-colors p-0.5"><Video size={18} /></button>
              <button type="button" onClick={() => setIsScheduleModalOpen(true)} className="hidden sm:inline-block hover:text-blue-600 text-slate-500 transition-colors p-0.5" title="Schedule a meeting">
