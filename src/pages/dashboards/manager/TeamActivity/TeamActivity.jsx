...
const TeamActivity = () => {
      ...
      if (task.assignees && Array.isArray(task.assignees)) {
        task.assignees.forEach((assignee) => {
          if (!memberMap.has(assignee.id)) {
            ...
            const doneTasks = projectData.Tasks.filter(t => 
              t.status === 'done' && t.assignees.some(a => a.id === assignee.id)
            ).length;
  ...
  // Dropdown list sets
  const allProjects = useMemo(() => ["All Projects", ...new Set(members.flatMap(m => m.projects))], [members]);
  const allRoles = useMemo(() => ["All Roles", ...new Set(members.map(m => m.role))], [members]);
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#fcfcfc]">
        ...
        <p className="text-xs font-bold text-slate-400">Loading live workspace activity...</p>
      </div>
    );
  }

  // Error Catch Screen State
  // if (error) {
  //   return (
  ...
  return (
    <div className="bg-[#fcfcfc] min-h-screen font-sans p-8">
          ...
          <table className="w-full text-left border-collapse min-w-[800px]">
            <thead>
              <tr className="bg-slate-50/50 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-50">
                ...
                <th className="px-6 py-4 text-center">Active Tasks (Done/Total)</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {filteredMembers.length === 0 ? (
                <tr>
                  <td colSpan="6" className="text-center py-8 text-xs font-bold text-slate-400">No active assignees found matching criteria.</td>
                </tr>
              ) : (
                filteredMembers.map((m, i) => (
      ...
      {selectedMember && (
        <TeamMemberModal member={selectedMember} onClose={() => setSelectedMember(null)} projectData={projectData} />
      )}
    </div>
  );
};
