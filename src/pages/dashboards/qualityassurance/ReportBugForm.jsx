...
const ReportBugForm = () => {
  ...
  const stepValidations = useMemo(() => {
    return [
      ...
      !!(formData.steps_to_repro.some(s => s.trim() !== "") && formData.expected_result && formData.actual_result), 
      true, 
      ...
      <div className="max-w-full mx-auto space-y-6 pb-20 animate-in fade-in-50 duration-500">
        ...
        <div className="flex justify-between items-center mb-6 md:mb-10">
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-400 mt-1">Log trace details directly into your workspace pipeline</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {currentStep === 0 && (
            <div className="bg-white rounded-md border border-slate-200 p-4 sm:p-6 md:p-8 shadow-sm animate-in fade-in slide-in-from-bottom-2 duration-300">
              ...
              <div className="grid gap-5 mt-6 md:mt-8">
                <InputField label="Bug Track Heading / Reference" name="title" value={formData.title} onChange={handleInputChange} placeholder="e.g. Critical Failure on Payment Process..." required />
                    ...
                    <div className="flex items-center gap-2">
                      {selectedAssignee ? (
                        <>
                          ...
                          <span className="truncate text-left">{selectedAssignee?.full_name}</span>
                        </>
                      ) : (
                        <span className="text-slate-400 font-normal">{isMembersLoading ? "Syncing Members..." : "Select Assignee Developer"}</span>
                      )}
                    </div>
                ...
                <div className="space-y-4">
                  ...
                  {formData.steps_to_repro.map((step, index) => (
                    <div key={index} className="flex gap-3 group items-center">
                      <span className="w-5 h-5 rounded-full bg-slate-800 text-white flex items-center justify-center text-[9px] font-black shrink-0">{index + 1}</span>
                      <input value={step} onChange={(e) => handleStepChange(index, e.target.value)} placeholder="Provide action trace context layer..." className="flex-1 border-b border-slate-100 text-xs py-2 focus:border-blue-500 outline-none font-medium text-slate-600 transition-colors" />
                      {formData.steps_to_repro.length > 1 && <Trash2 size={14} className="text-slate-300 hover:text-red-500 cursor-pointer transition-colors shrink-0" onClick={() => removeStep(index)}/>}
                    </div>
                  ))}
                </div>
            ...
            <div className="bg-white rounded-md border border-slate-200 p-4 sm:p-6 md:p-8 shadow-sm animate-in fade-in slide-in-from-bottom-2 duration-300">
              ...
              <div className="mt-6">
                <label className="border-2 border-dashed border-slate-200 rounded-md p-6 sm:p-10 flex flex-col items-center justify-center cursor-pointer hover:bg-slate-50/50 transition-colors group text-center">
                  <Upload className="w-8 h-8 text-slate-300 group-hover:text-blue-500 transition-colors mb-2" />
                  <p className="text-xs font-bold text-slate-500 px-2">Drop log captures or click to stream binary files</p>
                  <input type="file" multiple onChange={handleFileUpload} className="hidden" />
                </label>
                {files.length > 0 && (
                  <div className="mt-4 flex flex-wrap gap-2">
                    {files.map((f, i) => (
                      <div key={i} className="text-[10px] font-bold text-slate-600 bg-slate-100/80 border border-slate-200 px-3 py-1.5 rounded-md flex items-center gap-2 max-w-xs truncate">
                        <span className="truncate">{f.name} ({f.size})</span>
                        <X size={12} className="cursor-pointer text-slate-400 hover:text-red-500 transition-colors shrink-0" onClick={() => setFiles(p => p.filter((_, idx) => idx !== i))}/>
                      </div>
                    ))}
                  </div>
                )}
          ...
          {currentStep === 4 && (
            <div className={`rounded-md border-2 p-4 sm:p-6 md:p-8 transition-all animate-in fade-in slide-in-from-bottom-2 duration-300 ${formData.is_red_card ? 'border-red-500 bg-red-50/10' : 'border-slate-200 bg-white'}`}>
              <label className="flex items-center gap-3 cursor-pointer select-none">
                <input type="checkbox" checked={formData.is_red_card} onChange={(e) => setFormData(p => ({...p, is_red_card: e.target.checked}))} className="w-5 h-5 accent-red-500 rounded shrink-0" />
                <span className="font-black uppercase tracking-wider text-xs text-slate-700">Escalate Block Threat Layer (Raise Red Card)</span>
              </label>
              {formData.is_red_card && (
                <textarea name="red_card_reason" value={formData.red_card_reason} onChange={handleInputChange} className="w-full mt-4 p-4 bg-white border border-red-100 rounded-md text-xs font-medium text-slate-600 outline-none shadow-sm focus:border-red-400" placeholder="Provide absolute architecture blockers explanation..." required />
...
const SectionHeader = ({ title, sub }) => (
  <div className="flex items-center gap-3 border-b border-slate-50 pb-4">
    <div className="p-2 bg-blue-50 rounded-md text-blue-600 shadow-inner shrink-0"><CheckCircle size={18} /></div>
    <div>
      <h3 className="text-xs font-black uppercase tracking-wider text-slate-800 break-words">{title}</h3>
      <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest mt-0.5 break-words">{sub}</p>
    </div>
  </div>
);
